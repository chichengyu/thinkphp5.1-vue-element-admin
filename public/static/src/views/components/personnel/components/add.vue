<template>
    <div class="add" style="padding-right: 120px;">
            ...
            <Row :gutter="32">
                <Col>
                    <FormItem label="确认密码" prop="password_comfirm" label-position="top">
                        <Input type="password" v-model="formData.password_comfirm" placeholder="请再次输入密码" />
                    </FormItem>
                </Col>
            </Row>
            <Row :gutter="32">
        ...
        <div class="footer">
            <Button style="margin-right: 8px" @click.stop="showDrawer">返回</Button>
            <Button type="primary" :loading="loading" @click.stop="handleSubmit">提交</Button>
        </div>
    </div>
</template>
<script>
    import { add } from '@/api/admin/personnel.js'
    export default {
        props:['show','groups'],
        data () {
            return {
