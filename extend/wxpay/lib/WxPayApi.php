...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			...
			$inputObj->SetNotify_url($wxconfing['NOTIFY_URL']);//异步通知url
		}

//		$inputObj->SetAppid(WxPayConfig::APPID);//公众账号ID
//		$inputObj->SetMch_id(WxPayConfig::MCHID);//商户号
//		if(WxPayConfig::WXPAY_DEBUG) {
		$inputObj->SetAppid($wxconfing['APPID']);//公众账号ID
		$inputObj->SetMch_id($wxconfing['MCHID']);//商户号
		if($wxconfing['WXPAY_DEBUG']) {
...
//		$inputObj->SetMch_id(WxPayConfig::MCHID);//商户号
	...
	private static function reportCostTime($url, $startTimeStamp, $data)
	{
		...
		if($wxconfing['REPORT_LEVENL'] == 0){
			return;
		}
		//如果仅失败上报
		//if(WxPayConfig::REPORT_LEVENL == 1 &&
		if($wxconfing['REPORT_LEVENL'] == 1 &&
...
//			curl_setopt($ch,CURLOPT_PROXYPORT, WxPayConfig::CURL_PROXY_PORT);
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, $wxconfing['SSLCERT_PATH']);
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			//curl_setopt($ch,CURLOPT_SSLKEY, WxPayConfig::SSLKEY_PATH);
			curl_setopt($ch,CURLOPT_SSLKEY, $wxconfing['SSLKEY_PATH']);
		}
		//post提交方式
